...
class TestCoolingModule(unittest.TestCase):
    def setUp(self):
        ...
        self.plc.set_tag("FB200_EM_Cooling.UDT.CHW_Valve_Fdbk_AI", 0.0)
        self.plc.set_tag("FB200_EM_Cooling.UDT.CHW_Freeze_Alm", False)
        self.plc.set_tag("FB200_EM_Cooling.UDT.Valve_Failure_Alm", False)
    ...
    def run_cooling_logic(self):
        ...
        cmd = self.plc.get_tag("FB200_EM_Cooling.UDT.CHW_Valve_Cmd_AO")
        fdbk = self.plc.get_tag("FB200_EM_Cooling.UDT.CHW_Valve_Fdbk_AI")
    ...
    def test_freeze_safety_trip(self):
        ...
        self.run_cooling_logic()
        self.assertEqual(self.plc.get_tag("FB200_EM_Cooling.UDT.CHW_Valve_Cmd_AO"), 80.0)
        ...
        self.assertEqual(self.plc.get_tag("FB200_EM_Cooling.UDT.CHW_Valve_Cmd_AO"), 0.0)

    def test_valve_failure_alarm(self):
        """TC3: Test that the failure alarm triggers after a delay."""
        self.plc.set_tag("FB200_EM_Cooling.Enable", True)
        self.plc.set_tag("FB200_EM_Cooling.Valve_Demand_In", 50.0)
        self.run_cooling_logic() # Run once to set the command

        self.plc.set_tag("FB200_EM_Cooling.UDT.CHW_Valve_Fdbk_AI", 10.0) # Mismatched feedback

        # 1. First scan, timer is running but not complete
        self.mock_timer.Q = False
        self.run_cooling_logic()
        self.assertFalse(self.plc.get_tag("FB200_EM_Cooling.UDT.Valve_Failure_Alm"))

        # 2. Second scan, timer has completed
        self.mock_timer.Q = True
        self.run_cooling_logic()
        self.assertTrue(self.plc.get_tag("FB200_EM_Cooling.UDT.Valve_Failure_Alm"))
